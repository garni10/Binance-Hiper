...
st.markdown(
    "### 🏆 Identificación del Líder de Mercado (Modelo de Stackelberg)"
)
...
# Diferencias para aproximar estacionariedad
...
for vendor in top_10_vendedores:
    ...
    if vendor in df_granger_diff.columns:
        ...
        # Seleccionar las dos series
        data_test = df_granger_diff[
            ["Precio_Robusto", vendor]
        ].dropna()
        ...
        if len(data_test) > (max_lags * 5):
            ...
            try:
                ...
                # Buscar el lag con menor p-valor
                best_lag = None
                best_p_value = 1.0
                ...
                # Opcional: útil para diagnosticar
                # st.warning(
                #     f"No se pudo evaluar {vendor}: {e}"
                # )
...
# ====================================================
# 3. CREAR RANKING DE LÍDERES
# ====================================================

df_leaders_rank = pd.DataFrame(leader_results)

# IMPORTANTE:
# Solo ordenar si existen resultados
...
# ====================================================
# 4. IDENTIFICAR LÍDER Y ESTIMAR STACKELBERG
# ====================================================

if (
    not df_leaders_rank.empty
    and df_leaders_rank.iloc[0]["Es_Lider"]
):

    lider_detectado = df_leaders_rank.iloc[0]["Vendedor"]

    lag_optimo = int(
        df_leaders_rank.iloc[0]["Mejor_Lag"]
    )

    p_val_opt = (
        df_leaders_rank.iloc[0]["P_Valor"]
    )


    # ====================================================
    # 5. FUNCIÓN DE REACCIÓN DEL MERCADO
    # ====================================================

    df_stack = pd.DataFrame({

        "Y_Mercado":
            df_corr["Precio_Robusto_BUY"],
    ...
    st.success(
        f"**Líder de Precios Identificado:** "
        f"`{lider_detectado}`"
    )
    ...
    with col_a:
        ...
        st.metric(
            "Tiempo de Reacción",
            f"{lag_optimo} Snapshot(s)",
            help=(
                "Número de snapshots que le toma "
                "al mercado seguir al líder."
            )
        )
    ...
    with col_b:
        ...
        st.metric(
            "Sensibilidad / Coeficiente (β)",
            f"{beta_lider:.3f}",
            help=(
                "Por cada 1 BOB que ajusta el líder, "
                "el mercado reacciona cambiando β BOB."
            )
        )
    ...
    with col_c:
        ...
        st.metric(
            ...
            f"{r2_stack * 100:.1f}%",
            help=(
                "Porcentaje de variación del mercado "
                "explicada por la conducta del líder."
            )
        )


# ====================================================
# 7. SIN LÍDER ESTADÍSTICAMENTE SIGNIFICATIVO
# ====================================================

else:

    st.info(
        "No se encontró un único líder estadísticamente "
        "significativo (p < 0.05). El mercado responde a "
        "una dinámica de Bertrand atomizada/competitiva "
        "sin liderazgo persistente."
    )
